...
class localResolutionViewer(ProtocolViewer):
    ...
    def _showVolumeColorSlices(self, param=None):
        imageFile = self.protocol._getExtraPath(
            localResolutionViewer.OUTPUT_RESOLUTION_FILE, abs=True)
    ...
    def _plotHistogram(self, param=None):
        ...
        histogramFile = self.protocol._getExtraPath(OUTPUT_HISTOGRAM, abs=True)

        if exists(histogramFile):
            md = MetaData()
            md.read(histogramFile)
            x_axis = []
            y_axis = []
    ...
    def _plotVolumeSlices(self, title, volumeData, vminData, vmaxData,
        ...
        f.suptitle(title, fontsize=titleFontSize, color=titleColor,
                   fontweight='bold')
