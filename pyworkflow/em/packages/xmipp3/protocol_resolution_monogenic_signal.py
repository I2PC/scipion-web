...
class XmippProtMonoRes(ProtAnalysis3D):
    ...
    def _defineParams(self, form):
        ...
        form.addParam('noiseonlyinhalves', BooleanParam,
                      expertLevel=LEVEL_ADVANCED,
                      default=True,
    ...
    def resolutionMonogenicSignalStep(self):
        ...
        if self.filterInput.get():
            params += ' --filtered_volume %s' % self._getExtraPath(
                FN_FILTERED_MAP)
        else:
    ...
    def createHistrogram(self):
        ...
        params += ' --mask binary_file %s' % self._getExtraPath(
            OUTPUT_MASK_FILE)
        params += ' --steps %f' % 30
        params += ' --range %f %f' % (self.min_res_init,
                                      self.max_res_init)  # (self.minRes.get(), self.maxRes.get())
    ...
    def createOutputStep(self):
        ...
        if self.filterInput.get():
            ...
            if (self.halfVolumes):
                self.volumesSet2.setSamplingRate(
                    self.inputVolume.get().getSamplingRate())
            else:
                self.volumesSet2.setSamplingRate(
                    self.inputVolumes.get().getSamplingRate())
    ...
    def _summary(self):
        ...
        if (Nvox > 10):
            summary.append(
                "The resolution of %i %% of the mask voxels could not be computed. Maybe the mask was"
